  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 md:px-6 py-6 md:py-14 text-text-primary">
      ...
      {!loadingCourses && progressCourses.length === 0 && (
        <div ref={screenContainerRef} className="w-full max-w-2xl mx-auto px-6 py-24 text-center">
          <div className="w-14 h-14 mx-auto mb-6 rounded-2xl bg-accent-soft-bg border border-accent-soft-border flex items-center justify-center">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-6 h-6 text-brand"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
          </div>
          <h2 className="text-2xl font-display font-semibold text-text-primary mb-2">No courses yet.</h2>
          <p className="text-sm text-text-secondary mb-8 font-medium max-w-sm mx-auto leading-relaxed">Progress tracking isn't enabled for any course yet. Check back soon or visit the marketplace.</p>
      ...
      {progressCourses.length > 0 && !activeCourse && (
        <div ref={screenContainerRef} className="flex flex-col gap-8 md:gap-12">
          <div className="border-b border-border-default pb-6 md:pb-10">
            <span className="text-[11px] font-sans font-bold uppercase tracking-widest text-brand block mb-2">Your Courses</span>
            <h1 className="text-3xl md:text-5xl font-display font-semibold text-text-primary tracking-tight leading-tight">Track your progress.</h1>
            <p className="text-text-secondary text-sm mt-3 font-medium max-w-lg">Pick a course below to view your topic-by-topic study checklist and related PYQs.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {progressCourses.map((course) => (
              ...
              >
                <div className={`h-1 w-full ${course.unlocked ? 'bg-brand' : 'bg-border-subtle'}`} />
                <div className="p-5 md:p-6 flex flex-col grow">
                  <div className="flex items-start justify-between gap-2 mb-3">
                    <span className="inline-block text-[9px] font-bold text-brand bg-accent-soft-bg border border-accent-soft-border rounded-full px-2.5 py-0.5 uppercase tracking-wider shrink-0">
                      {course.subject}
                    </span>
                    {!course.unlocked && (
                      <span className="flex items-center gap-1 text-[9px] font-bold text-status-warning-text bg-status-warning-bg border border-status-warning-text/25 rounded-full px-2 py-0.5 uppercase tracking-wide shrink-0">
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-2.5 h-2.5"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                        Locked
                      </span>
                    )}
                  </div>
                  <h3 className="font-display font-semibold text-text-primary text-sm md:text-[15px] group-hover:text-brand transition-colors leading-snug grow">{course.name}</h3>
                </div>
                <div className="px-5 md:px-6 pb-5 md:pb-6 pt-3 border-t border-border-default flex items-center justify-between mt-auto">
                  <span className="text-[10px] text-text-tertiary font-bold uppercase tracking-wider flex items-center gap-1">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3 h-3"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
                    {getFileEntries(course).length} PDF{getFileEntries(course).length !== 1 ? 's' : ''}
                  </span>
                  {course.unlocked ? (
      ...
      {activeCourse && selectedFileIndex === null && (
        <div ref={screenContainerRef} className="flex flex-col gap-8 md:gap-12">
          <div className="border-b border-border-default pb-6 md:pb-10 flex flex-wrap items-start justify-between gap-4">
            <div>
              <span className="text-[11px] font-sans font-bold uppercase tracking-widest text-brand block mb-2">{activeCourse.subject}</span>
              <h1 className="text-2xl md:text-4xl font-display font-semibold text-text-primary tracking-tight leading-tight">{activeCourse.name}</h1>
              <p className="text-text-secondary text-sm mt-2 font-medium">Select a PDF to view its checklist.</p>
            </div>
            <button
                ...
                >
                  <div className="p-5 md:p-6">
                    <div className="flex items-start justify-between gap-3 mb-3">
                      <p className="font-display font-semibold text-text-primary text-sm md:text-[15px] group-hover:text-brand transition-colors leading-snug">{entry.name}</p>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-4 h-4 text-brand shrink-0 mt-0.5 transition-transform group-hover:translate-x-1 duration-200"><line x1="5" y1="12" x2="19" y2="12"/><polyline points="12 5 19 12 12 19"/></svg>
                    </div>
                    {stats && !stats.loading ? (
                      <>
                        ...
                        <p className="text-[11px] text-text-tertiary font-semibold tabular-nums">{stats.totalCompleted}/{stats.totalQuestions} done · {pct}%</p>
                      </>
                    ) : (
        ...
        <div className={`transition-[padding] duration-300 ${pyqPanelOpen ? 'pl-80 sm:pl-96' : 'pl-0'}`}>
          ...
          <div className="sticky top-0 z-30 bg-page/95 backdrop-blur-md border-b border-border-default">
            <div className="max-w-4xl mx-auto px-5 md:px-6 py-3 flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <span className="text-xs font-extrabold text-brand tabular-nums">{percent}%</span>
                <span className="text-[10px] text-text-tertiary font-semibold tabular-nums hidden sm:inline">{totalCompleted}/{totalQuestions}</span>
                {getFileEntries(activeCourse).length > 1 && (
                  <button
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className="w-3 h-3"><polyline points="15 18 9 12 15 6"/></svg>
                  Courses
                </button>
              </div>
            </div>
          ...
          <div className="max-w-4xl mx-auto px-6 py-8 flex flex-col gap-6">
            ...
            {!loadingTopics && topics.map((topic) => {
              ...
              const doneCount = topic.questions.filter(q => q.completed).length;
              const topicPct = topic.questions.length > 0 ? Math.round((doneCount / topic.questions.length) * 100) : 0;
              const borderAccent = doneCount === 0 ? 'border-border-default' : doneCount === topic.questions.length ? 'border-brand' : 'border-brand/40';
              return (
              <div key={topic._id} className={`bg-surface border ${borderAccent} rounded-2xl overflow-hidden shadow-sm transition-colors duration-300`}>
                <button
                ...
                >
                  <div className="flex items-center gap-2.5 min-w-0">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" className={`w-3.5 h-3.5 text-text-tertiary shrink-0 transition-transform duration-200 ${isCollapsed ? '-rotate-90' : ''}`}><polyline points="6 9 12 15 18 9"/></svg>
                    <h3 className="font-display font-semibold text-text-primary text-sm leading-snug truncate">{topic.name}</h3>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-3">
                    {topicPct === 100 && (
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" className="w-3.5 h-3.5 text-brand"><polyline points="20 6 9 17 4 12"/></svg>
                    )}
                    <span className={`text-[10px] font-bold tabular-nums ${topicPct === 100 ? 'text-brand' : 'text-text-tertiary'}`}>
                ...
                <div className="divide-y divide-border-subtle">
                  {topic.questions.map((q) => (
                    ...
                    >
                      ...
                      >
                        {q.completed && (
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="3.5" strokeLinecap="round" strokeLinejoin="round" className="w-2.5 h-2.5"><polyline points="20 6 9 17 4 12"/></svg>
                        )}
                      </button>
                      <div className="min-w-0 flex-1">
                        <p className={`text-[13px] leading-relaxed font-medium ${q.completed ? 'text-text-tertiary line-through decoration-text-tertiary/60' : 'text-text-primary'}`}>{q.questionText}</p>
                        <div className="flex items-center gap-2 mt-1.5 flex-wrap">
                          <span className="text-[10px] text-text-tertiary font-semibold tabular-nums">pg. {q.pageNumber}</span>
                          {splitTagDisplay(q.tag).map((t, idx) => (
                            <span key={idx} className="text-[9px] font-bold text-brand bg-accent-soft-bg border border-accent-soft-border rounded-full px-2 py-0.5">{t}</span>
                          ))}
                        </div>
                      </div>
                    </div>
            ...
            <div className="px-5 py-4 border-b border-border-default flex items-center justify-between shrink-0">
              <div>
                ...
                {pyqPanelItems.length > 0 && (
                  <p className="text-[10px] text-text-tertiary font-medium mt-0.5">{pyqPanelItems.filter(p => p.completed).length}/{pyqPanelItems.length} completed</p>
                )}
              </div>
              <button onClick={() => setPyqPanelOpen(false)} className="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-sunken text-text-secondary hover:text-text-primary cursor-pointer transition-colors">
            ...
            <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-4">
              ...
              {!pyqPanelLoading && pyqPanelItems.length === 0 && (
                <div className="text-center pt-8">
                  <p className="text-xs text-text-tertiary font-medium leading-relaxed">Complete a question to see related PYQs here.</p>
                </div>
              )}
...
function PyqPanelItem({ pyq, onToggle }) {
  return (
    <div className={`border rounded-xl p-3.5 flex gap-2.5 transition-colors ${pyq.completed ? 'bg-surface border-border-subtle' : 'bg-sunken border-border-default'}`}>
      <button
      ...
      <span className={`shrink-0 mt-0.5 text-[10px] font-extrabold rounded-lg px-2 py-0.5 h-fit tabular-nums ${pyq.completed ? 'bg-sunken text-text-tertiary' : 'bg-accent-soft-bg text-brand border border-accent-soft-border'}`}>
        {pyq.year}
      </span>
      <p className={`text-[12px] leading-relaxed flex-1 min-w-0 ${pyq.completed ? 'text-text-tertiary line-through decoration-text-tertiary/50' : 'text-text-secondary'}`}>{pyq.questionText}</p>
    </div>
  );
}
