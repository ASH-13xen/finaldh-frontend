...
export default function PurchaseCourses({ user, onUserUpdate }) {
  ...
  const [samplePageNumber, setSamplePageNumber] = useState(1);
  const [sampleNumPages, setSampleNumPages] = useState(null);
  const [samplePdfError, setSamplePdfError] = useState(false);
  const [samplePdfWidth, setSamplePdfWidth] = useState(700);
  const sampleSectionRef = useRef(null);
  const samplePdfContainerRef = useRef(null);

  // Track the PDF viewer container width for responsive rendering
  useEffect(() => {
    if (!samplePdfContainerRef.current) return;
    const observer = new ResizeObserver(([entry]) => {
      setSamplePdfWidth(entry.contentRect.width);
    });
    observer.observe(samplePdfContainerRef.current);
    return () => observer.disconnect();
  }, [activeSampleCourse]);

  const handleSeeSample = (course) => {
    setActiveSampleCourse(course);
    setSamplePageNumber(1);
    setSampleNumPages(null);
    setSamplePdfError(false);
    setTimeout(() => {
  ...
  const sampleStatusPendingRequest = activeSampleCourse
    ? purchaseRequests.find(
        (r) => r.courseId === activeSampleCourse.courseId && r.status === "pending",
      )
    : null;
  const sampleSubjectDisplay = activeSampleCourse
    ? (activeSampleCourse.subject.startsWith("GS-")
        ? activeSampleCourse.subject
        : OPTIONAL_NAMES[activeSampleCourse.subject]?.replace("Optional: ", "") ||
          activeSampleCourse.subject)
    : "";
  ...
  return (
    <div className="w-full max-w-6xl mx-auto px-4 md:px-6 py-6 md:py-14">
      ...
      <div
        ...
        className="mt-12 md:mt-16 pt-10 border-t border-slate-800"
      >
        <div className="mb-6">
          <h2 className="text-base md:text-xl font-extrabold text-white tracking-tight">
            Course Sample Preview
            ...
            <div className="lg:w-[65%] border border-slate-800 rounded-2xl overflow-hidden">
              <div className="px-4 md:px-6 py-4 flex items-center justify-between border-b border-slate-800 bg-slate-900/60 flex-wrap gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  {sampleNumPages && (
                    <span className="text-[10px] text-slate-500 font-medium shrink-0">({sampleNumPages} pages)</span>
                  )}
                </div>
                {sampleNumPages && (
                  <div className="flex items-center gap-1.5 shrink-0">
                    <button
                      onClick={() => setSamplePageNumber((p) => Math.max(1, p - 1))}
                      disabled={samplePageNumber <= 1}
                    ...
                    <span className="text-[11px] text-slate-400 font-medium min-w-[60px] text-center">
                      {samplePageNumber} / {sampleNumPages}
                    </span>
                    <button
                      onClick={() => setSamplePageNumber((p) => Math.min(sampleNumPages, p + 1))}
                      disabled={samplePageNumber >= sampleNumPages}
                      className="px-2 py-1 bg-slate-800 hover:bg-slate-700 disabled:opacity-30 border border-slate-700 text-slate-300 rounded-lg text-[11px] font-bold transition cursor-pointer disabled:cursor-default"
                    >
                      ›
                    </button>
              ...
              <div ref={samplePdfContainerRef} className="bg-slate-950 flex justify-center py-6 min-h-[500px]">
                {samplePdfError ? (
                  ...
                  <Document
                    ...
                    onLoadError={() => setSamplePdfError(true)}
                    loading={
                  ...
                  >
                    <Page
                      pageNumber={samplePageNumber}
                      width={Math.min(samplePdfWidth - 48, 700)}
                ...
                <div>
                  {sampleStatus.type === "purchased" ? (
                    ...
                    <div className="space-y-2">
                      ...
                      {sampleStatusPendingRequest && sampleStatusPendingRequest.telegramNotificationCount < 2 && (
                        <button
                          onClick={(e) => handleTelegramNotify(sampleStatusPendingRequest, activeSampleCourse, e)}
                          className="w-full px-3 py-2 bg-accent-600 hover:bg-accent-550 text-white rounded-xl text-[10px] font-bold transition flex items-center justify-center gap-1.5 cursor-pointer shadow-sm"
                        >
                          Notify Admin on Telegram
                        </button>
                    ...
                    <button
                      onClick={() => handleOpenPurchaseModal(activeSampleCourse)}
                    ...
                    >
                      ...
                      {sampleStatus.type === "rejected" ? "Retry Purchase" : "Purchase Now"}
                    </button>
                  )}
                </div>
              </div>
