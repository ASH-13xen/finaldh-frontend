...
export default function PurchaseCourses({ user, onUserUpdate }) {
  ...
  const handleFormSubmit = async (e) => {
    ...
    if (!upiTxnId.trim()) {
      setModalError('Please enter the UPI Transaction ID.');
      return;
    }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {courses.map((course) => {
            ...
            return (
              <div 
                key={course._id} 
                className="bg-slate-900/40 backdrop-blur-md border border-slate-800 hover:border-slate-700 rounded-2xl p-6 shadow-xl flex flex-col justify-between hover:shadow-indigo-950/10 hover:-translate-y-0.5 transition-all duration-300 relative overflow-hidden group"
              >
                <div className="space-y-4">
                  {/* Category Badge */}
                  <span className="text-[9px] font-extrabold text-indigo-400 bg-indigo-950/40 border border-indigo-900/60 rounded px-2 py-0.5 uppercase tracking-wider w-fit block">
                    {subjectDisplay}
                  </span>
                  
                  {/* Course Name */}
                  <h3 className="text-sm md:text-base font-bold text-slate-100 group-hover:text-white line-clamp-2 leading-relaxed transition-colors">
                ...
                <div className="border-t border-slate-800 pt-4 mt-6 flex items-center justify-between">
                  <div>
                    ...
                    <span className="text-lg font-extrabold text-slate-100">₹{course.price || 499}</span>
                  </div>
                  ...
                  {status.type === 'purchased' ? (
                    ...
                    <div className="flex items-center gap-1.5 px-3 py-1.5 bg-amber-500/10 border border-amber-500/20 text-amber-400 rounded-xl text-[10px] md:text-xs font-bold animate-pulse">
                    ...
                    <button
                      ...
                      className="px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition-all shadow-md hover:shadow-indigo-950/20 cursor-pointer flex items-center gap-1.5"
                    >
                      ...
                      {status.type === 'rejected' ? 'Retry Purchase' : 'Purchase Now'}
                    </button>
                  )}
                </div>
              </div>
              ...
              <form onSubmit={handleFormSubmit} className="space-y-5">
                ...
                <div className="bg-slate-950/80 border border-slate-850 rounded-2xl p-4 md:p-5 flex flex-col items-center text-center space-y-4 relative overflow-hidden">
                  ...
                  <p className="text-xs text-slate-300 font-semibold leading-relaxed">
                    Scan the QR code below using GPay, PhonePe, Paytm, or any UPI app to transfer <span className="text-indigo-400 font-extrabold text-sm">₹{selectedCourse.price || 499}</span>. Click the QR code to view it full size.
                  </p>
                  ...
                  <p className="text-[10px] text-slate-450 font-medium">
                    Payee: The Dark Horse UPSC | UPI ID: mockpayment@upipays
                  </p>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] md:text-xs font-bold text-slate-400 uppercase tracking-wider">
                      UPI Transaction ID / Ref No. (12-Digit)
                    </label>
                    <input
                      type="text"
                      maxLength={24}
