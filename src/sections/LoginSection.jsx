...
export default function LoginSection({ error, googleButtonRef, onDevLogin }) {
  ...
  // Vite sets import.meta.env.DEV=true only for `npm run dev` builds, never for
  // production `npm run build` output — so this block cannot ship to prod.
  const showDevLogin = import.meta.env.DEV && typeof onDevLogin === 'function';
    ...
    <div className="min-h-screen bg-page text-text-primary flex flex-col lg:flex-row">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-10 lg:py-0 bg-surface-raised lg:border-l border-border-default">
        <div ref={cardRef} className="w-full max-w-md bg-surface border border-border-default rounded-2xl p-8 shadow-xl flex flex-col">
          ...
          {showDevLogin && (
            <div className="mb-6 p-3.5 bg-status-warning-bg/40 border border-dashed border-status-warning-text/40 rounded-xl">
              <p className="text-[11px] font-sans font-bold uppercase tracking-wide text-status-warning-text mb-2">Local dev only</p>
              <input
                type="email"
                value={devEmail}
                onChange={(e) => setDevEmail(e.target.value)}
                placeholder="admin email (optional)"
                className="w-full mb-2 bg-surface border border-border-default rounded-lg px-3 py-2 text-xs text-text-primary focus:outline-none focus:border-brand"
              />
              <button
                type="button"
                onClick={() => onDevLogin(devEmail.trim())}
                className="w-full py-2 bg-brand hover:bg-brand-hover text-text-on-accent rounded-lg text-xs font-sans font-bold transition-all cursor-pointer"
              >
                Continue as developer
              </button>
              <p className="text-[10px] text-text-tertiary mt-2">Needs <code>ALLOW_DEV_LOGIN=true</code> in the backend .env.</p>
            </div>
          )}
